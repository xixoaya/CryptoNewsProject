...
function Home({ Username, OnSignOut, OnDelete, OnStartFlow, OnEndFlow, OnShowModal }) {
    ...
    const clearDetail = () => {setbulletinId(null)}
    ...
    return <div className="pagelayout">
        ...
        {view === 'home' && <>
            ...
            {!bulletinId && <BulletinsCoverLogged OnBackHome={clearDetail} onItem={goToDetail} OnStartFlow={OnStartFlow} 
                OnEndFlow={OnEndFlow} OnShowModal={OnShowModal} ></BulletinsCoverLogged>
            }

            {bulletinId && <Detail name={name} itemid={bulletinId} itemQueue={bulletinQueue} OnBackList={() => setbulletinId(null)} 
                OnStartFlow={OnStartFlow} OnEndFlow={OnEndFlow} OnShowModal={OnShowModal} ></Detail>
            }
        </>}

        {view === 'profile' && <Profile
            name={name} OnBackHome={goToHome} OnSignOut={OnSignOut} OnDelete={OnDelete} OnStartFlow={OnStartFlow} 
            OnEndFlow={OnEndFlow} OnShowModal={OnShowModal}></Profile>
        }

        {view === 'search' && <SearchBulletins
            name={name} OnBackHome={goToHome} OnSignOut={OnSignOut} OnDelete={OnDelete} OnStartFlow={OnStartFlow} 
            OnEndFlow={OnEndFlow} OnShowModal={OnShowModal}></SearchBulletins>
        }
